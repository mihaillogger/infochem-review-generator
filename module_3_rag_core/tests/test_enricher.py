...
from rag_core.schemas import Paragraph, ParsedDocument, Section, StandardSection


@pytest.mark.asyncio
@patch("rag_core.enricher.settings.USE_LLM_ENRICHMENT", True)
@patch("rag_core.enricher.settings.GEMINI_API_KEY", "test_key")
@patch("rag_core.enricher.settings.GEMINI_API_URL", "test_url")
...
@pytest.mark.asyncio
@patch("rag_core.enricher.settings.USE_LLM_ENRICHMENT", False)
