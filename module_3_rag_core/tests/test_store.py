...
import numpy as np
import pytest
from rag_core.schemas import DBChunk, DBChunkMetadata
from rag_core.store import AsyncQdrantStore


@pytest.mark.asyncio
@patch("rag_core.store.AsyncQdrantClient")
@patch("rag_core.store.NomicEmbedder")
@patch("rag_core.store.SparseTextEmbedding")
